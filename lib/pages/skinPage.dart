...
class SkinPage extends StatefulWidget {
  const SkinPage({super.key, required this.title});
  final String title;
  @override
...
class _SkinPageState extends State<SkinPage> {
  ...
  Future<void> loadProductWithBrandData() async {
    ...
    List<Product> products = await ProductService().retrieveProductsByCategoryId('S06QeCJdPDMn7E4LavRK');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          children: [
            Text('\nNOS PRODUITS TEINTS', style: TextStyle(fontWeight: FontWeight.bold,
                color: Colors.red[200]!,
                fontSize: 20)),
            const Row(
              children: [
